...
- **`wrangler.toml`** — Worker name `localed`, `nodejs_compat`, assets from `.open-next/assets`. Published site files use Supabase Storage (no R2). `compatibility_date` must be `2024-09-23` or later. For a custom domain, use Workers & Pages → your worker → Custom domains.
...
| `PUBLISHED_SITES_CDN_URL` | Optional. Override base URL for published site static files. When unset, the app uses Supabase Storage public URL from `NEXT_PUBLIC_SUPABASE_URL`. |

### Published sites: Supabase Storage

Published site pages are **static HTML** from the EJS template. The app stores them in **Supabase Storage** (no full page in the DB).
...
2. Leave **`PUBLISHED_SITES_CDN_URL`** unset. The app uses  
...
| Published sites not redirecting / 404 on view site | Create a **public** Storage bucket named `published-sites` in the Supabase dashboard; ensure `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are set. |
...
- [ ] **Published sites:** Supabase Storage bucket `published-sites` created and set to **Public** (section 4)
